...
# -- Depends on :
#       Output of preprocess_trajectories.R (saved as image ../DerivedData/raw_trajectories.RData)
#
# -- Produces :
  ...
  for ( i in 1:length( allIDs ) ) {
    ...
    if ( noFurtherFlag == 0 ) {
      ...
      if( length( idxEver ) == 0 ) {
                ...
                status.sx  <- NA
                status.sof <- NA
                status.TRS <- 1    # censored - never met all / full TRS criteria
      ...
      if ( length( idxEver ) > 0 ) {
        ...
        # -- percentage change in symptoms : Leucht et al, 2009 corrected : denominator is BASELINE, not followup as stated in Leucht. 
        # -- see : https://onlinelibrary.wiley.com/doi/10.1111/j.1600-0447.2011.01720.x
        deltaPANSS <- 100 * ( baselinePANSS[ c("adjPos", "adjNeg", "adjTot") ] - followupPANSS[ c("adjPos", "adjNeg", "adjTot") ] ) /
                                baselinePANSS[ c("adjPos", "adjNeg", "adjTot") ]
...
write.csv( tab.Surv, file = paste0( data.path, "survivalData_tabulated.csv"), row.names = FALSE )
