...
rm(list = ls())
require(dplyr)
root.path <- "./"           # -- NB : this script should be executed from ./Code/
...
# -- Dump out the processed trajectory data and environment for later use
    save( TRS.sx.traj, TRS.sofas.traj, TRS.rx.traj, 
          allIDs, keyvars, demog.tab,
          PANSS.tab, pmhx.tab, SCID.tab, root.path, data.path,
          file = paste0( data.path, "raw_trajectories.RData" ) )


# -- clean environment for next script, keeping only necessary dataframes / variables
# keepVars <- c("TRS.sx.traj","TRS.rx.traj","TRS.sofas.traj","allIDs","keyvars","demog.tab",
#               "PANSS.tab","pmhx.tab","SCID.tab","root.path","data.path")
